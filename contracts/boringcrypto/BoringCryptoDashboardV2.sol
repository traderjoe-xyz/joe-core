...
contract BoringCryptoDashboardV2
{
    ...
    constructor(address _chef, address _pangolinFactory, address _joeFactory, address _wavax) public {
    ...
    function getAVAXRate(address token) public view returns(uint256) {
        ...
        if (token != wavax)
        {
            ...
            uint112 reserve0Pangolin; uint112 reserve1Pangolin;
            uint112 reserve0Joe; uint112 reserve1Joe;
            ...
            if (address(pairJoe) == address(0) || reserve0Pangolin > reserve0Joe || reserve1Pangolin > reserve1Joe) {
                if (pairPangolin.token0() == wavax) { avax_rate = uint256(reserve1Pangolin).mul(1e18).div(reserve0Pangolin); } else { avax_rate = uint256(reserve0Pangolin).mul(1e18).div(reserve1Pangolin); }
            } else {
                if (pairJoe.token0() == wavax) { avax_rate = uint256(reserve1Joe).mul(1e18).div(reserve0Joe); } else { avax_rate = uint256(reserve0Joe).mul(1e18).div(reserve1Joe); }
            }
        }
        return avax_rate;
    }    
