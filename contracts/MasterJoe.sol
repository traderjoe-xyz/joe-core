...
interface IMigratorJoe {
    // Perform LP token migration from legacy UniswapV2 to JoeSwap.
...
contract MasterJoe is Ownable {
    ...
    struct UserInfo {
        ...
        //   1. The pool's `accJoePerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock; // Last block number that JOEs distribution occurs.
    ...
    // Block number when bonus JOE period ends.
    uint256 public bonusEndBlock;
    // JOE tokens created per block.
    uint256 public joePerBlock;
    ...
    // The block number when JOE mining starts.
    ...
    constructor(
        JoeToken _joe,
        address _devaddr,
        uint256 _joePerBlock,
        uint256 _startBlock,
        uint256 _bonusEndBlock
    ) public {
        joe = _joe;
        devaddr = _devaddr;
        joePerBlock = _joePerBlock;
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 joeReward =
                multiplier.mul(joePerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
    ...
    function updatePool(uint256 _pid) public {
        ...
        joe.mint(devaddr, joeReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accJoePerShare).div(1e12).sub(
                    user.rewardDebt
                );
