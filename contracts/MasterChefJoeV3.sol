...
contract MasterChefJoeV3 is Ownable {
    ...
    function harvest(uint256 pid, address to) public {
        PoolInfo memory pool = updatePool(pid);
        UserInfo storage user = userInfo[pid][msg.sender];
        uint256 accumulatedJoe = user.amount.mul(pool.accJoePerShare) / ACC_JOE_PRECISION;
        uint256 _pendingJoe = accumulatedJoe.sub(user.rewardDebt);
    ...
    /// @notice Withdraw LP tokens from MCJV3 and harvest proceeds for transaction sender to `to`.
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param amount LP token amount to withdraw.
    /// @param to Receiver of the LP tokens and JOE rewards.
    function withdrawAndHarvest(
        ...
        // Effects
        user.rewardDebt = accumulatedJoe.sub(amount.mul(pool.accJoePerShare) / ACC_JOE_PRECISION);
        user.amount = user.amount.sub(amount);
