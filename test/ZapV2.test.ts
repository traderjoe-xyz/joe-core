...
describe("zapV2", function () {
    ...
    describe("Revert", function () {
        it("Revert on zapIn", async function () {
            ...
            await expect(this.zapV2.connect(this.alice).zapIn(
                "0",
                "41500000",
        ...
        it("Revert on zapInTokens", async function () {
            ...
            await expect(this.zapV2.connect(this.alice).zapInToken(
                this.wavax.address,
                "1000000000000000000",
                "41500000",
                "0",
        ...
        it("Revert on zapOut", async function () {
            await this.zapV2.connect(this.alice).zapIn(
                0,
                0,
            ...
            await expect(this.zapV2.connect(this.alice).zapOutToken(
    ...
    describe("ZapIn", function () {
        it("zap 1 AVAX to JOE/WAVAX and zapOut to JOE", async function () {
            ...
            await this.joeAvax.connect(this.alice).approve(this.zapV2.address, "10000000000000000000000000000")
            await this.zapV2.connect(this.alice).zapOutToken(
                this.joeAvax.address,
                await this.joeAvax.balanceOf(this.alice.address),
                0,
                [JOE_ADDRESS],
        ...
        it("zap 1 AVAX to USDC/USDT and zapOut to AVAX, testing the 2 paths inverted", async function () {
            ...
            await this.zapV2.connect(this.alice).zapOut(
        ...
        it("zap JOE to APEX/AVAX and zapOut to USDT", async function () {
            await this.router.connect(this.alice).swapExactAVAXForTokens(
                "0",
                [WAVAX_ADDRESS, JOE_ADDRESS],
                this.alice.address,
                "100000000000000000",
            ...
            await this.zapV2.connect(this.alice).zapInToken(
                JOE_ADDRESS,
                balance,
                0,
                0,
                [JOE_ADDRESS, WAVAX_ADDRESS, APEX_ADDRESS],
