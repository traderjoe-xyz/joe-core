...
describe("JoeBar", function () {
  ...
  beforeEach(async function () {
    ...
    this.bar = await this.JoeBar.deploy(this.joe.address)
    this.joe.mint(this.alice.address, "100")
    this.joe.mint(this.bob.address, "100")
    this.joe.mint(this.carol.address, "100")
  })

  it("should not allow enter if not enough approve", async function () {
    await expect(this.bar.enter("100")).to.be.revertedWith("ERC20: transfer amount exceeds allowance")
    await this.joe.approve(this.bar.address, "50")
    await expect(this.bar.enter("100")).to.be.revertedWith("ERC20: transfer amount exceeds allowance")
    await this.joe.approve(this.bar.address, "100")
  ...
  it("should work with more than one participant", async function () {
    await this.joe.approve(this.bar.address, "100")
    await this.joe.connect(this.bob).approve(this.bar.address, "100", { from: this.bob.address })
    ...
    expect(await this.joe.balanceOf(this.bar.address)).to.equal("30")
    // JoeBar get 20 more JOEs from an external source.
    await this.joe.connect(this.carol).transfer(this.bar.address, "20", { from: this.carol.address })
    // Alice deposits 10 more JOEs. She should receive 10*30/50 = 6 shares.
