...
describe("Migrator", function () {
  before(async function () {
    ...
    this.JoeFactory = await ethers.getContractFactory("JoeFactory")
  ...
  beforeEach(async function () {
    this.factory1 = await this.JoeFactory.deploy(this.alice.address)
    await this.factory1.deployed()

    this.factory2 = await this.JoeFactory.deploy(this.alice.address)
    await this.factory2.deployed()

    this.joe = await this.JoeToken.deploy()
    await this.joe.deployed()

    this.weth = await this.ERC20Mock.deploy("WAVAX", "WAVAX", "100000000")
    ...
    this.lp1 = await this.JoePair.attach((await pair1.wait()).events[0].args.pair)
  ...
  it("should allow first minting from public only after migrator is gone", async function () {
    ...
    this.lpx = await this.JoePair.attach((await pair.wait()).events[0].args.pair)
